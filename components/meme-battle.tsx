    ...
    <div className="overflow-hidden rounded-2xl border-2 border-red-900/50 bg-card shadow-2xl">
      <div className="border-b border-red-900/30 bg-gradient-to-r from-red-950/30 via-red-900/10 to-red-950/30 px-6 py-4">
        <div className="flex items-center justify-center gap-3">
          ...
          <h3 className="font-sans text-xl font-black uppercase tracking-tight text-red-500" style={{ fontFamily: 'Impact, "Arial Black", sans-serif' }}>MEME BATTLE ROYALE</h3>
          <Trophy className="h-6 w-6 text-red-500" />
        </div>
        <p className="mt-2 text-center text-sm font-bold text-red-400/70">WHICH SOLUTION IS MORE INSANE?!</p>
      </div>
      ...
      <div className="grid gap-4 p-6 md:grid-cols-2">
        ...
        >
          ...
          <div className="border-t border-red-900/30 bg-red-950/30 px-4 py-3 text-center">
            <p className="text-2xl font-black text-red-500">{battle.left.votes + (voted === "left" ? 1 : 0)}</p>
        ...
        >
          ...
          <div className="border-t border-red-900/30 bg-red-950/30 px-4 py-3 text-center">
            <p className="text-2xl font-black text-red-500">{battle.right.votes + (voted === "right" ? 1 : 0)}</p>
            <p className="text-xs font-bold uppercase tracking-wider text-red-400/60">VOTES</p>
          </div>
        </div>
      </div>
      ...
      {voted && (
        <div className="border-t border-red-500/30 bg-red-900/30 px-6 py-4 text-center">
          <p className="font-black text-red-400">VOTE RECORDED! NEXT BATTLE IN 5:32</p>
        </div>
      )}
    </div>
  )
